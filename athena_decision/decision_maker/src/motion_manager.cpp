...
: athena_decision::CascadeManager("motion_manager", "")
{
...
void MotionManager::checkout_mode()
{
  ...
  uint8_t inter_changed_(athena_decision::DEFAULT);
  ...
  if (automation_manager_node_->get_current_state().id() ==  // NOLINT
    lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE ||
    automation_manager_node_->chainnodes_state_ == athena_decision::PART_DEACTIVE)
  {
    auto current_stamp = this->get_clock()->now();
    automation_manager_node_->auto_check(athena_utils::CHECK_TO_START);  // if part_deactive
    automation_manager_node_->auto_check(athena_utils::CHECK_TO_PAUSE);
    while (automation_manager_node_->chainnodes_state_ != athena_decision::ALL_DEACTIVE &&
  ...
  while (rclcpp::ok() && new_request) {
    if (mode_server_->is_cancel_requested()) {
      ...
      if (next_mode.control_mode >= Mode_T::MODE_SEMI) {
        ...
        if (automation_manager_node_->get_current_state().id() ==  // NOLINT
          lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE ||
          automation_manager_node_->chainnodes_state_ != athena_decision::ALL_DEACTIVE)
        {
          automation_manager_node_->auto_check(athena_utils::CHECK_TO_PAUSE);
        }
      }
    ...
    if (new_request) {
      switch (next_mode.control_mode) {
        ...
        case Mode_T::MODE_MANUAL: {
            ...
            if (condition_pre) {
              if (check_inside) {
                ...
                {
                  if (inter_changed_ == athena_decision::SUCCEED) {
                    message_info(
                      TAG_MODE +
                      std::string("Auto ") +
                      ...
                      std::string(" succeed"));
                    new_request = false;
                  } else if (inter_changed_ == athena_decision::FAILED) {
                    message_info(
                      TAG_MODE +
                      std::string("Auto ") +
                      gait_label_[mode_trig_gait.gait] +
                    ...
                    switch (result.code) {
                      case rclcpp_action::ResultCode::SUCCEEDED: {
                          if (result.result.get()->succeed) {
                            inter_changed_ = athena_decision::SUCCEED;
                          } else {
                            inter_changed_ = athena_decision::FAILED;
                          }
                          break;
                        }
        ...
        case Mode_T::MODE_TRACK: {
            ...
            if (!check_submode) {
              if (automation_manager_node_->chainnodes_state_ == athena_decision::ALL_ACTIVE) {
                message_info(
                  TAG_MODE +
                ...
                lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE)
              {
                ...
                automation_manager_node_->auto_check(athena_utils::CHECK_TO_START);
                feedback->timestamp = this->get_clock()->now();
              ...
              if (check_inside) {
                if (this->get_clock()->now() - current_time <=
                  std::chrono::seconds(timeout_manager_))
                {
                  if (inter_changed_ != athena_decision::DEFAULT) {
                    if (inter_changed_ == athena_decision::SUCCEED) {
                      message_info(
                        TAG_MODE +
                ...
                auto client_callback =
                  [&](rclcpp::Client<SubMode_T>::SharedFuture future) {
                    inter_changed_ = athena_decision::DEFAULT;
  ...
  if (result->err_code == ModeRes_T::NORMAL) {
      ...
      result->err_code != ModeRes_T::AVOID_PREEMPT)
    {
      ...
      inter_check_gait_sync(recover_gait, timeout_gait_, athena_utils::MODE_TRIG);
    }
...
void MotionManager::checkout_gait()
{
  ...
  if (running_mode.control_mode <= Mode_T::MODE_LOCK &&
    ...
    current_motivation != athena_utils::MODE_TRIG)
  {
    message_warn(
      TAG_GAIT +
  ...
  if (new_request) {
    ...
    while (rclcpp::ok() && gait_list.size() > 0) {
        ...
        gait_to_pub.gait != Gait_T::GAIT_DEFAULT)
      {
        ...
        publish_gait(gait_to_pub, current_motivation == athena_utils::ORDER_REQ);
...
void MotionManager::mon_order_exec()
{
      ...
      case MonOrder_T::MONO_ORDER_TURN_AROUND:
        {
          if (gait_cached_.gait != order_to_gait_[goal_order.id]) {
            ...
            auto check_result = inter_check_gait_sync(
              gait_to_use, timeout_gait_, athena_utils::ORDER_REQ);
            if (!check_result) {
              message_info(
                TAG_ORDER +
      ...
      case MonOrder_T::MONO_ORDER_SHOW:
        {
          ...
          if (!std::experimental::filesystem::exists(order_file)) {
            ...
            if (!inter_check_gait_sync(gait_to_use, timeout_gait_, athena_utils::ORDER_REQ)) {
              result->err_code = MonOrderRes_T::FAILED;
              message_info(
                TAG_ORDER +
    ...
    result->err_code != MonOrderRes_T::UNAVAILABLE)
  {
    ...
    gait_to_use.timestamp = this->get_clock()->now();
    if (!inter_check_gait_sync(gait_to_use, timeout_gait_, athena_utils::GAIT_TRIG)) {
      result->err_code = MonOrderRes_T::RECOV_FAILED;
    }
  }
...
void MotionManager::init()
{
  ...
  automation_manager_node_ =
    std::make_shared<athena_decision::AutomationManager>("multi");
