...
#include "athena_utils/simple_action_server.hpp"
...
using LifecycleNode_T = athena_utils::LifecycleNode;
...
// enums
using GaitChangePriority_T = athena_utils::GaitChangePriority;
...
class MotionManager : public athena_decision::CascadeManager
{
public:
  MotionManager();
  ~MotionManager();
...
// Action Server
  std::unique_ptr<athena_utils::SimpleActionServer<ChangeMode_T, LifecycleNode_T>> mode_server_;
  std::unique_ptr<athena_utils::SimpleActionServer<ChangeGait_T,
    LifecycleNode_T>> gait_server_;
  std::unique_ptr<athena_utils::SimpleActionServer<ExtMonOrder_T,
...
// Node individual
  std::shared_ptr<athena_decision::AutomationManager> automation_manager_node_;
...
// Log maps
  ...
  {
    {athena_utils::LOCK_DETECT, std::string("lock_detect")},
    {athena_utils::MODE_TRIG, std::string("mode_trig")},
    {athena_utils::GAIT_TRIG, std::string("pattern_trig")},
    {athena_utils::ORDER_REQ, std::string("order_req")}
  };
