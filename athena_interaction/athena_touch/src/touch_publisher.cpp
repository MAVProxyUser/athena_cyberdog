...
namespace athena_touch
{
class TouchPubNode : public athena_utils::LifecycleNode
{
public:
  TouchPubNode();
  ~TouchPubNode();
...
: athena_utils::LifecycleNode("TouchPubNode")
{
...
athena_utils::CallbackReturn TouchPubNode::on_configure(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
...
athena_utils::CallbackReturn TouchPubNode::on_activate(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  RCLCPP_INFO(get_logger(), "Activaing");
  publisher_->on_activate();
  isthreadrunning = true;
  touch_receiver_thread = std::thread(&athena_touch::TouchPubNode::touch_receiver_callback, this);
  return athena_utils::CallbackReturn::SUCCESS;
}

athena_utils::CallbackReturn TouchPubNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
...
athena_utils::CallbackReturn TouchPubNode::on_cleanup(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  publisher_.reset();
  return athena_utils::CallbackReturn::SUCCESS;
...
athena_utils::CallbackReturn TouchPubNode::on_shutdown(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  publisher_.reset();
  return athena_utils::CallbackReturn::SUCCESS;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<athena_touch::TouchPubNode>();
