...
{
VoiceCmd::VoiceCmd()
: athena_utils::LifecycleNode("voice_cmd")
{
...
athena_utils::CallbackReturn VoiceCmd::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  wake_led_client_ = this->create_client<SensorNodeT>(
    "athena_led", rmw_qos_profile_system_default,
    callback_group_);
...
athena_utils::CallbackReturn VoiceCmd::on_activate(const rclcpp_lifecycle::State &)
{
...
athena_utils::CallbackReturn VoiceCmd::on_deactivate(const rclcpp_lifecycle::State &)
{
...
athena_utils::CallbackReturn VoiceCmd::on_cleanup(const rclcpp_lifecycle::State &)
{
...
athena_utils::CallbackReturn VoiceCmd::on_shutdown(const rclcpp_lifecycle::State &)
{
  ...
  while (rclcpp::ok()) {
    ...
    if (new_goal) {
      ...
      while (!play_end) {
        ...
        // feedback->feed.status = athena_interfaces::msg::AudioFeedback::REJECT;
        // feedback->feed.status = athena_interfaces::msg::AudioFeedback::ERROR;
...
// RCLCPP_COMPONENTS_REGISTER_NODE(athena_voicecmd::VoiceCmdActionClient)
