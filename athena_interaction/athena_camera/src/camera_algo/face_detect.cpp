...
bool FaceDetect::init()
{
  ...
  athena_camera::FaceManager::getInstance();

  m_jpegEncoder = std::make_shared<athena_camera::JpegEncoder>(
...
bool FaceDetect::processImageBuffer(ImageBuffer & buffer)
{
  ...
  if (m_addFace) {
    ...
    FeatureMap endlib_feats = athena_camera::FaceManager::getInstance()->getFeatures();
    {
...
void FaceDetect::processFaceResult(ImageBuffer & frame, std::vector<MatchedFaceInfo> & faces)
{
  ...
  for (size_t i = 0; i < msg->count; i++) {
    ...
    info.is_host = athena_camera::FaceManager::getInstance()->isHost(info.id);
