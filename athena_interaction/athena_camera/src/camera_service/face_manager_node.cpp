...
int FaceManagerNode::addFaceInfo(std::string & args)
{
  ...
  ret = athena_camera::FaceManager::getInstance()->addFaceInfo(name, is_host);
...
{
  return athena_camera::FaceManager::getInstance()->cancelAddFace(args);
}

int FaceManagerNode::confirmLastFace()
{
  return athena_camera::FaceManager::getInstance()->confirmFace();
}
...
int FaceManagerNode::updateFaceId(std::string & args)
{
  ...
  return athena_camera::FaceManager::getInstance()->updateFaceId(ori_name, new_name);
}
...
int FaceManagerNode::deleteFace(std::string & face_name)
{
  ...
  return athena_camera::FaceManager::getInstance()->deleteFace(face_name);
}
...
int FaceManagerNode::getAllFaces(std::shared_ptr<FaceManagerT::Response> response)
{
  ...
  for (unsigned int i = 0; i < filenames.size(); i++) {
    ...
    if (athena_camera::FaceManager::getInstance()->isHost(face_name)) {
