...
#include "athena_utils/can/can_proto.h"
...
namespace athena_utils
{

athena_sensor::athena_sensor(int sensor_count)
{
...
athena_sensor::~athena_sensor()
{
  free(filter_id_table);
}

void
athena_sensor::get_regulater_name(int boardID)
{
  DIR * dir;
  struct dirent * ptr;
...
int
athena_sensor::set_convert_sensortype(uint16_t sensortype_bitmask)
{
...
int
athena_sensor::clear_convert_sensortype(uint16_t sensortype_bitmask)
{
  publish_sensor_bitmask &= ~sensortype_bitmask;
  return 0;
}
...
int
athena_sensor::get_sensor_data_size(int sensortype)
{
  switch (sensortype) {
...
int
athena_sensor::find_sensor_filter_stdid()
{
...
uint8_t
athena_sensor::find_sensor_boardID(int sensor_type)
{
  switch (sensor_type) {
    case SENSOR_TYPE_ACCELEROMETER:
    case SENSOR_TYPE_MAGNETIC_FIELD:
...
uint64_t
athena_sensor::timer_converter_board2nv(uint8_t boardID, uint64_t stm32_timestamp)
{
  switch (boardID) {
    case 3:
...
uint64_t
athena_sensor::get_current_nv_time()
{
  struct timespec ts;
...
bool
athena_sensor::convert_timestamp_data(struct can_frame * recv_frame)
{
  bool ret = false;
...
bool
athena_sensor::check_sensor_data_valid(
...
bool
athena_sensor::convert_sensor_data(
  sensor_data_converter * sensor_data_converter,
...
int
athena_sensor::board_regulator_control(int board_id, int enable)
{
  int fd = -1, ret = -1;
...
void
athena_sensor::log_prev_sensor_data(sensors_event_t sensor_data, int sensortype)
{
  memcpy(&previous_sensor_data[sensortype], &sensor_data, sizeof(sensors_event_t));
}

bool
athena_sensor::sensor_data_heart_attack_checker(int sensortype)
{
