...
#include "athena_utils/can/can_utils.hpp"
#include "athena_utils/sensor/sensor_utils.hpp"
...
namespace athena_body_state
{
class BodyStatePubNode : public athena_utils::LifecycleNode
{
public:
  BodyStatePubNode();
  ~BodyStatePubNode();
...
private:
  ...
  athena_utils::can_dev_operation * can_dev_op = nullptr;
  athena_utils::can_dev_operation * can_tim_op = nullptr;
  athena_utils::athena_sensor * sensor_obj = nullptr;
...
: athena_utils::LifecycleNode("BodyStatePubNode")
{
...
athena_utils::CallbackReturn BodyStatePubNode::on_configure(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  ...
  can_dev_op = new athena_utils::can_dev_operation();
  can_tim_op = new athena_utils::can_dev_operation();
  sensor_obj = new athena_utils::athena_sensor(2);
...
athena_utils::CallbackReturn BodyStatePubNode::on_activate(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  ...
  can_data_receiver_thread = std::thread(
    &athena_body_state::BodyStatePubNode::can_data_receiver_callback, this);
  can_timestamp_receiver_thread = std::thread(
    &athena_body_state::BodyStatePubNode::can_timestamp_receiver_callback, this);
...
athena_utils::CallbackReturn BodyStatePubNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
...
athena_utils::CallbackReturn BodyStatePubNode::on_cleanup(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  publisher_.reset();
  return athena_utils::CallbackReturn::SUCCESS;
...
athena_utils::CallbackReturn BodyStatePubNode::on_shutdown(const rclcpp_lifecycle::State & state)
{
  UNUSED_VAR(state);
  publisher_.reset();
  return athena_utils::CallbackReturn::SUCCESS;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<athena_body_state::BodyStatePubNode>();
